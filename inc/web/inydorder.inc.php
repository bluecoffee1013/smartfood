...
if(checksubmit('submit')){
    ...
    $sql="select * from ".tablename("wpdc_ydorder")."where uniacid=:uniacid and store_id=:store_id";
        if($type=='all'){
            if(checksubmit('submit2')){
                $sql.=" and time2 >= :time2 and time2 <= :time22";
        $data=array(':time2'=>$start,':time22'=>$end,':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
        $total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('wpdc_ydorder')." WHERE time2 >= :time2 and time2 <= :time22 and uniacid=:uniacid  and store_id=:store_id ORDER BY time2 DESC",$data);
        }elseif(checksubmit('submit')){
            $sql.="  and (link_name LIKE :name|| order_num LIKE :order_num)   ";
            $data=array(':name' => $where,':order_num' => $where,':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('wpdc_ydorder')." WHERE (link_name LIKE :name || order_num LIKE :order_num)  and uniacid=:uniacid   and store_id=:store_id ORDER BY time2 DESC",$data);
        }else{
            $sql=$sql;
            $data=array(':uniacid'=>$_W['uniacid'],':store_id'=>$storeid);
            $total=pdo_fetchcolumn("select count(*) from " .tablename('wpdc_ydorder')."where uniacid=:uniacid   and store_id=:store_id",$data);
